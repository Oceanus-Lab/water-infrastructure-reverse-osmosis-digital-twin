  ...
  return (
    ...
    >
      <span className="relative flex items-start gap-2 py-1">
        ...
        {task.note ? (
          <span className="shrink-0 text-muted-foreground text-xs tabular-nums font-mono">
            {task.note}
          </span>
        ) : null}
 ...
 * in the whole panel.
 */
export function AITaskList({ className, label = "Plan", tasks }: AITaskListProps) {
  ...
  return (
    <div
      className={cn(
        "w-full rounded-xl border border-border bg-background/90 p-3",
        className
      )}
    >
      <div className="mb-1.5 flex items-baseline justify-between">
        <p className="font-medium text-foreground text-sm">{label}</p>
        <p className="text-muted-foreground text-xs tabular-nums font-mono">
          {done}/{all.length}
        </p>
      </div>
