...
- **FR-022**: The feature MAY corroborate the measured leading indicator with a modeled scaling-propensity mechanism (e.g. saturation indices for CaSO₄ and silica that rise as recovery concentrates the feed), but any such corroboration MUST be labeled modeled — it is computed on an assumed feed-chemistry profile, since the dataset carries no scaling-ion measurements — and MUST NOT substitute for, reweight, or alter the measured ranking. The leading indicator remains earned from the history (evidence-first); the mechanism only explains a candidate cause.
...
- **Scaling mechanism is modeled corroboration only, never a measured claim**: The OCWD data carries pH, conductivity/EC, turbidity, and TOC but no individual scaling ions (Ca, Mg, SO₄, HCO₃, SiO₂, Ba, Sr). Any saturation-index / scaling-propensity narrative therefore rests on an assumed literature feed-chemistry profile and is labeled modeled per Constitution Principle IV. It may corroborate *why* the measured leading indicator behaves as it does, but the ranking, lead time, precision, and baseline error remain measured from the history (Principle II, evidence-first).
...
- **Assumed feed-chemistry reference profile (modeled input, only if scaling corroboration is used)**: The optional scaling-propensity corroboration (FR-022) consumes a literature-sourced OCWD/GWRS feed-ion profile — a modeled reference input, not part of the measured dataset — established at planning time and labeled modeled wherever it appears.
